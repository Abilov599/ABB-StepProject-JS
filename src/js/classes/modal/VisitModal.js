...
class VisitModal extends Modal {
  ...
  createInput(labelText, type = "text") {
    ...
    input.setAttribute(
      checkType ? "placeholder" : "rows",
      checkType ? labelText : "3"
    );
  ...
  listenSelect(form, e) {
    ...
    if (e.target.value === "Cardiologist") {
      ...
      const bloodPressure =
        document?.getElementById("blood-pressure")?.parentElement;
      const bodyMass =
        document?.getElementById("body-mass-index")?.parentElement;
      const diseases = document?.getElementById(
  ...
  loadForm() {
    ...
    const doctors = this.createSelectList(
      "Select a doctor",
      this.#doctors,
      "doctors"
    );
    const levels = this.createSelectList(
      "Select a level",
      this.#levels,
      "levels"
    );
